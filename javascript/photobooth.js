...
const draw = () => {
 ...
 let filterString = "none";
  ...
  if (currentFilter === 1) filterString = "none";
  else if (currentFilter === 2) filterString = "grayscale(100%)";
  else if (currentFilter === 3) filterString = "sepia(100%)";
  else if (currentFilter === 4) filterString = "invert(100%)";
  else if (currentFilter === 5) filterString = "brightness(1.5) saturate(1.5)";
  ...
  if(photoStage < 4){
    ...
    if (currentFilter > 1) {
      ...
      for (let i = 0; i < data.length; i += 4) {
        ...
        if (currentFilter === 2) { // Grayscale
          const avg = (r + g + b) / 3;
          data[i] = data[i + 1] = data[i + 2] = avg;
        } 
        else if (currentFilter === 3) { // Sepia formula
          data[i]     = (r * 0.393) + (g * 0.769) + (b * 0.189);
          data[i + 1] = (r * 0.349) + (g * 0.686) + (b * 0.168);
          data[i + 2] = (r * 0.272) + (g * 0.534) + (b * 0.131);
        } 
        else if (currentFilter === 4) { // Invert formula
          data[i]     = 255 - r;
          data[i + 1] = 255 - g;
          data[i + 2] = 255 - b;
        }
        else if (currentFilter === 5) { // Brighten (approx 1.5x)
